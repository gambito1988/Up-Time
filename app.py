...
def init_db():
    ...
    with get_db() as connection:
        ...
        for table in ("users", "service_records", "membership_payments", "contact_messages"):
...
def notify_contact(name, email, phone, message):
    """Avisa al negocio por email de un mensaje nuevo (si CONTACT_NOTIFY_EMAIL está definido)."""
    recipient = os.environ.get("CONTACT_NOTIFY_EMAIL")
    if not recipient:
        return
    # Con EMAIL_WEBHOOK_URL el aviso sale por n8n (HTTPS); si no, por SMTP.
    sender = send_email_via_webhook if os.environ.get("EMAIL_WEBHOOK_URL") else send_email
    sender(
        recipient,
        f"Nuevo contacto de {name} - Up Time",
        f"Nombre: {name}\nEmail: {email}\nTeléfono: {phone or '-'}\n\n{message}\n\n"
        "Lo puedes ver y marcar como respondido en el panel privado.",
    )
...
@app.post("/contacto")
@csrf.exempt  # Se envía desde la landing estática (otro origen); guarda el mensaje y redirige a WhatsApp.
@limiter.limit("10 per hour")
def contacto():
    if request.form.get("website"):  # campo señuelo: solo lo completan los bots
        return redirect(url_for("home") + "#contacto"), 303
    nombre = " ".join(request.form.get("nombre", "").split())
    email = request.form.get("email", "").strip()
    telefono = " ".join(request.form.get("telefono", "").split())
    mensaje = request.form.get("mensaje", "").strip()
    if not nombre or not email or not mensaje:
        return redirect(url_for("home") + "#contacto"), 400
    values = {"nombre": nombre, "email": email, "telefono": telefono, "mensaje": mensaje}
    if any(len(values[field]) > limit for field, limit in CONTACT_LIMITS.items()):
        return redirect(url_for("home") + "#contacto"), 400
    try:
    ...
    # Si la base falla igual se redirige a WhatsApp: el visitante no debe perder el contacto.
    try:
        save_contact_message(nombre, email, telefono or None, mensaje)
    except DATABASE_ERRORS:
        app.logger.exception("No se pudo guardar el mensaje de contacto")
    run_in_background(notify_contact, nombre, email, telefono, mensaje)
...
def send_email_via_webhook(to, subject, body):
    """Envía el email a través del workflow de n8n: POST JSON con la clave en un header."""
    ...
    request_data = Request(
        ...
        headers={"Content-Type": "application/json", "X-Uptime-Secret": secret},
        method="POST",
    )
    try:
        ...
        app.logger.warning("No se pudo enviar el email '%s' por webhook: %s", subject, error)
        return False
    return True
